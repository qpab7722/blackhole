...
int Mirr_overheattime[4] = { -1 };//과열 상태  변수
...
void UpOB()	//돌출 지형을 일정 간격마다 위로 올려주는 함수 
{
	...
	{
		memcpy(&GBInfo_N[y][1], &GBInfo_N[y + 1][1], GBOARD_WIDTH * sizeof(int));//내장함수,복사할  많이들 사용
	}
}
...
int main(void)
{
	...
	while (1)
	{
		...
		if (changeMap_Normal == false && changeMap_Boss == true && check_B>0)	//레이저를 쏘Y	//check_B는 보스맵을 다 그리고서 레이저를 쏘려고
		{
